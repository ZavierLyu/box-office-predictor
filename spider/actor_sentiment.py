                ...
                for p in text.findChildren():
                    if has_class(p, "tweet-text"):
                        ...
                        for content in p.contents:
                            if hasattr(p, "contents") and not isinstance(content, type(p)):
                                tweet_text.append(clean_text(content))
                        tweet.text = " ".join(tweet_text)
    ...
    if crawler.depth == 1:
        if os.path.exists(crawler.output_file):
            ...
            with open(crawler.output_file, "wt") as f:
                ...
                f.write("\n")
            with open('movie_name.csv', "a",newline = "") as f:
                f.write("{}".format(crawler.output_file))
                f.write("\n")

    parameters = crawler.parameters

    with open(crawler.output_file, "at", encoding='utf-8') as f:
        for (i, parameter) in enumerate(parameters):
            if hasattr(tweet, parameter):
                if type(getattr(tweet, parameter)) == list:
                    string = " ".join(getattr(tweet, parameter))
                    f.write(str(string))
                else:
...
class TwitterCrawler:
    ...
    def __init__(self, query="hoge", lang='en', max_depth=None, parser=parse_html, 
                    ...
                    init_min_pos=None, output_file="output", max_num=None,
    ...
    def check_if_finished(self):
        ...
        if self.max_depth is not None and self.depth >= self.max_depth:
            return True
        if self.max_num is not None and self.num >= self.max_num:
            print("maximum num: {}".format(self.num))
            return True
        else:
            return False
...
# with open('movie_metadata.csv','rt', encoding='utf-8') as csvfile:
#     reader = csv.DictReader(csvfile)
#     actor = [row['actor_1_name'] for row in reader]
# with open('movie_metadata.csv','rt', encoding='utf-8') as csvfile:
#     reader = csv.DictReader(csvfile)
#     year = [row['title_year'] for row in reader]

# for i in range(1):
#     if director[i] != '':
#         query = "{} since:2019-11-01 until:2019-11-02".format(director[i])
#         print("processing {}:{}".format(i,director[i]))
#         crawler = TwitterCrawler(query=query, output_file="{}.csv".format(director[i]),max_num=10)
#         crawler.crawl()
...
def main(input_path, output_path):
    ...
    year = df["year"].tolist()
    actor = df["movie_title"].tolist()

    result_data = {"actor":[], "sentiment":[], "count":[]}
    ...
    for i in range(len(df)):
        if year[i] != '':
            ...
            if not (os.path.isfile(savePath)):
                continue
            try:
                df = pd.read_csv(savePath, error_bad_lines=False)
            except pd.errors.ParserError:
                continue
            # print(df["text"])
            sen_data = 0
            for z in range(len(df)):
                text = str(df.iloc[z]["text"])
                # print("text:", text)
                testimonial = TextBlob(text)
                sen_data += testimonial.sentiment.polarity
            sen_avg = sen_data / len(df)
            result_data["actor"].append(actor[i])
            result_data["sentiment"].append(sen_avg)
            result_data["count"].append(len(df))
            print(f"actor:{actor[i]} sentiment:{sen_avg} count:{len(df)}")
        else:
            continue
